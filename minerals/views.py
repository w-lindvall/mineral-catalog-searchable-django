...
def mineral_name_search(request):
    ...
    groups = ['Silicates', 'Oxides', 'Sulfates', 'Sulfides',
              'Carbonates', 'Halides', 'Sulfosalts', 'Phosphates',
...
def mineral_group_search(request, group):
    ...
    all_minerals = Mineral.objects.all()
    random_mineral = random.choice(all_minerals)
    minerals = Mineral.objects.filter(group__contains=group).values('name',
                                                                    'pk')
    return render(request, 'index.html', {'minerals': minerals,
                                          'active_group': group,
                                          'groups': groups,
...
def mineral_first_search(request, letter='a'):
    ...
    all_minerals = Mineral.objects.all()
    random_mineral = random.choice(all_minerals)
    minerals = Mineral.objects.filter(name__istartswith=letter).values('name',
                                                                       'pk')
    return render(request, 'index.html', {'minerals': minerals,
